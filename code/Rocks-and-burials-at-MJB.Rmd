...
Is the association of rocks and burials at MKII non-random? A test using resampling methods and geometric morphometry
========================================================
Ben Marwick
...
sw <- ddply(areas, "type", summarize, 
                pval = shapiro.test(area)$p.value, 
                stat = shapiro.test(area)$statistic)
...
pandoc.table(sw, style="rmarkdown")
```
...
windows()
BEST_plot <- plotAll(BESTout, credMass=0.95, ROPEm=c(-0.1,0.1),
 ROPEeff=c(-0.2,0.2), compValm=0.5, oneGrp=TRUE)
 win.metafile(filename = "BEST_out.wmf", restoreConsole = TRUE)
 dev.off()
# link to download of image b/c this one doesn't work
# 800 x 1200 
```
...
Investigate difference in means with statistics
...
```{r, echo=FALSE, eval=FALSE }
# just for the BEST plot ???????????????? how to capture it as a file?
...
There is no credible difference in the sizes of rocks on skulls compared to rocks not on skulls.

Shapes of rocks on skulls compared to rocks not on skulls.
...
(sk_mnv <- manova.Coe(sk_r_o_center_F, "fac"))
